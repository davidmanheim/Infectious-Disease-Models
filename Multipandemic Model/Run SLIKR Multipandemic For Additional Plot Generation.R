...
CreatePlots=FALSE # If I want it to made the PDFs.
...
#For looking at the max/min. This is after running the other plots script for D=3
Paper_Output_Graphs=FALSE
if(D==3&&Paper_Output_Graphs==TRUE){
  #Only one of the following two; worst case, or best case.
  #Case_num = which((Ellipses_DF[,1]-Ellipses_DF[,7])==max((Ellipses_DF[,1]-Ellipses_DF[,7])))
  #Case_num = which((Ellipses_DF[,1]-Ellipses_DF[,7])==min((Ellipses_DF[,1]-Ellipses_DF[,7])))
  p_s=Multipandemic_List[[Case_num]]
  Param_Combo=list(D=D, min_scary=Overall_variables$min_scary, max_scary=Overall_variables$max_scary, 
                   contact_rate=Overall_variables$contact_rate, distancing=Overall_variables$distancing,
                   continue_distancing=Overall_variables$continue_distancing,
                   C=as.array(C), p_s=Multipandemic_List[[Case_num]], C_List=as.list(C_List))
}  
